...
import extract_features_module
...
def print_pressed_keys(e):
    try:
        ...
        if (pid[-1] > 0):
            ...
            if press == 'down':
                file_logs.write(str(current_time()) + ",Key Press,{0},{1}\n".format(key, foreground_exec))
            elif press == 'up':
                file_logs.write(str(current_time()) + ",Key Release,{0},{1}\n".format(key, foreground_exec))
    except:
        pass
...
def on_move(x, y):
    try:
        ...
        if (pid[-1] > 0):
            foreground_exec = psutil.Process(pid[-1]).name()
            file_logs.write(str(current_time()) + ",Mouse Movement,{0},{1},{2}\n".format(x, y, foreground_exec))
    except:
        pass
...
def on_scroll(x, y, dx, dy):
    try:
        ...
        if (pid[-1] > 0):
            foreground_exec = psutil.Process(pid[-1]).name()
            file_logs.write(
                str(current_time()) + ",Mouse Scroll,{0},{1},{2},{3},{4}\n".format(x, y, dx, dy, foreground_exec))
    except:
        pass
...
def launch():
    ...
    while ListenerMouse.is_alive():
        ...
        if timestamp + window * 1000 < current_time():
            timestamp = current_time()
            file_logs.close()
            window = extract_features_module.extract_features(path, path_apps, window)
        ...
        #     result = extract_features_module.obtain_auth_puntuation()
        #     text_notification = ""
        #
        #     # Show notification
        #     if result == 999:
